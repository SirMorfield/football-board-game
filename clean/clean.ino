...
//   if ( gamestatus == OFF && (panINP[0] & (1 << 0)))
// 	{
// 		gamestatus = ON;
// 		for (i = 0 ; i < 8 ; i++)
// 			paneel ( 0, i, 0 ) ;
...
bool snooze_patern()
{
	...
	for (i = 1; i <= NKOL; i++)
	{
		set_panel_light(kol, head, 1); // Set set_panel_lightl
		if (head > 3)
			set_panel_light(kol, head - 4, 0);
		if (head < 4)
			set_panel_light(kol, 7 - 3 + head, 0);
	}
